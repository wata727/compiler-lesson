...
static Type *func_params(Token **rest, Token *tok, Type *ty) {
  ...
  while (!equal(tok, ")")) {
    ...
    // "array of T" is converted to "pointer to T" only in parameter context 
...
// struct-initializer2 = initialize ("," initializer)*
...
static Node *funcall(Token **rest, Token *tok) {
  ...
  while (!equal(tok, ")")) {
    ...
    if (param_ty) {
      ...
      // If parameter type is omitted, float arguments are promoted to double.
